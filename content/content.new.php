<?php
	...
	Class contentExtensionFilebrowserNew extends AdministrationPage{
...
*/
		...
		function action(){
			...
			switch($_POST['type']){
				case 'upload':
					...
					redirect(extension_filebrowser::baseURL() . 'browse/' . (is_array($this->_context) && !empty($this->_context) ? implode('/', $this->_context) . '/' : NULL));
					break;
				...
				case 'directory':
					...
					array_shift($this->_context);
					redirect(extension_filebrowser::baseURL() . 'browse/' . (is_array($this->_context) && !empty($this->_context) ? implode('/', $this->_context) . '/' : NULL) . '/' . $_POST['fields']['directory']['name']);
					break;
		...
		function view(){
			...
			$this->Form->prependChild(Widget::Input('MAX_FILE_SIZE', Symphony::Configuration()->get('max_upload_size', 'admin'), 'hidden'));
			...
			if(isset($_POST['fields'])){
				...
				$fields['file']['permissions'] = Symphony::Configuration()->get('write_mode', 'file');
				$fields['upload']['permissions'] = Symphony::Configuration()->get('write_mode', 'file');
				$fields['directory']['permissions'] = Symphony::Configuration()->get('write_mode', 'directory');
			}
